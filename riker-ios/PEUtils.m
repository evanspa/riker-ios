...
+ (NSString *)labelTextForRecordCount:(NSInteger)recordCount {
  ...
  return [NSString stringWithFormat:@"%@ record%@", [formatter stringFromNumber:@(recordCount)], trailerForLabel];
}
...
+ (NSDecimalNumber *)millisecondsFromDate:(NSDate *)date {
...
+ (NSDate *)dateFromString:(NSString *)dateValue
  ...
  if ([PEUtils isNotNil:dateValue] && dateValue.length > 0) {
...
+ (NSComparisonResult)mscompareDate:(NSDate *)date1 toDate:(NSDate *)date2 {
  NSDecimalNumber *(^rounded)(NSDate *) = ^ NSDecimalNumber *(NSDate *date) {
    NSDecimalNumber *dateTime = [[NSDecimalNumber alloc] initWithDouble:[date timeIntervalSince1970]];
    return [dateTime decimalNumberByRoundingAccordingToBehavior:[NSDecimalNumberHandler decimalNumberHandlerWithRoundingMode:NSRoundPlain scale:0 raiseOnExactness:NO raiseOnOverflow:NO raiseOnUnderflow:NO raiseOnDivideByZero:NO]];
  };
  NSDecimalNumber *date1Time = rounded(date1);
  NSDecimalNumber *date2Time = rounded(date2);
  return [date1Time compare:date2Time];
}
